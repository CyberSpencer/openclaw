...
function renderInsightList(
  ...
  return html`
    <div class="usage-insight-card">
      ...
      ${
        items.length === 0
          ? html`<div class="muted" style="padding: 10px 0;">${emptyLabel}</div>`
          : html`
              <div class="usage-insight-list">
                ${items.map(
                  (item) => html`
                    <div class="usage-insight-item">
                      <div>
                        <div class="usage-insight-label">${item.label}</div>
                        ${item.sub ? html`<div class="usage-insight-sub">${item.sub}</div>` : nothing}
                      </div>
                      <div class="usage-insight-value">${item.value}</div>
                    </div>
                  `,
                )}
              </div>
