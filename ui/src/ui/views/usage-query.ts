...
import { UsageSessionEntry, UsageAggregates } from "./usageTypes.ts";
...
export const buildQuerySuggestions = (
  query: string,
...
export const applySuggestionToQuery = (query: string, suggestion: string): string => {
  const trimmed = query.trim();
  if (!trimmed) {
    return `${suggestion} `;
  }
...
export const normalizeQueryText = (value: string): string => value.trim().toLowerCase();

export const addQueryToken = (query: string, token: string): string => {
  const trimmed = query.trim();
  if (!trimmed) {
    return `${token} `;
  }
...
export const removeQueryToken = (query: string, token: string): string => {
  const tokens = query.trim().split(/\s+/).filter(Boolean);
  const next = tokens.filter((entry) => entry !== token);
  return next.length ? `${next.join(" ")} ` : "";
};

export const setQueryTokensForKey = (query: string, key: string, values: string[]): string => {
