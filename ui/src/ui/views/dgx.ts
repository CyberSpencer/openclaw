...
const SERVICE_META: Record<string, { label: string; sub: string; port?: number }> = {
  // 4 required core services
  router: { label: "vLLM Spark", sub: "Intent Routing", port: 8001 },
  ...
  // fallback path keys
  vllm_nemotron: { label: "vLLM Spark", sub: "Nemotron Inference", port: 8004 },
...
export function renderDgx(props: DgxProps): TemplateResult {
  ...
  return html`
    <div class="dgx-dashboard">
      ...
      <!-- Core Services (the 4 required: vLLM Spark, Qdrant, Embeddings, Reranker) -->
      ${
