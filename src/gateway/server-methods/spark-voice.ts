...
export const sparkVoiceHandlers: GatewayRequestHandlers = {
  ...
  "spark.voice.stt": async ({ respond, params }) => {
    try {
      ...
      const body = {
        ...
        format: (params?.format as string) || "webm",
        sample_rate: (params?.sample_rate as number) || 16000,
   ...
   * Request: { text: string; voice?: string; format?: string }
  ...
  "spark.voice.tts": async ({ respond, params }) => {
    try {
      ...
      console.warn(
        `[spark-voice] TTS failed: textLen=${textLen} url=${urlSafe} timeoutSec=${timeoutSec} aborted=${isAborted}`,
      );
