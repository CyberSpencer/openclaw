...
export const DEFAULT_GATEWAY_PORT = 32555;
...
export function resolveEventsLogDir(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  return path.join(stateDir, "logs", "events");
}

export function resolveEventsLogPath(
  date: Date = new Date(),
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  return path.join(resolveEventsLogDir(env, stateDir), `${formatLocalDate(date)}.ndjson`);
}

export function resolveEventsLogPathForDay(
  day: string,
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  return path.join(resolveEventsLogDir(env, stateDir), `${day}.ndjson`);
}
