...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(32555)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(32555)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(32555);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(32555, {
      timeoutMs: 500,
      intervalMs: 100,
