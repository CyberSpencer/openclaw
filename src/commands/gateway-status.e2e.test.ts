...
const loadConfig = vi.fn(() => ({
  gateway: {
    mode: "remote",
    remote: { url: "ws://remote.example:32555", token: "rtok" },
    auth: { token: "ltok" },
  },
}));
const resolveGatewayPort = vi.fn(() => 32555);
  ...
  it("infers SSH target from gateway.remote.url and ssh config", async () => {
    ...
    await withUserEnv("steipete", async () => {
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://peters-mac-studio-1.sheep-coho.ts.net:32555", token: "rtok" },
        },
      });
  ...
  it("falls back to host-only when USER is missing and ssh config is unavailable", async () => {
    ...
    await withUserEnv("", async () => {
      loadConfig.mockReturnValueOnce({
        gateway: {
          mode: "remote",
          remote: { url: "ws://studio.example:32555", token: "rtok" },
        },
      });
